...
int campos_potenciales(float intensity,float *light_values,movement *movements,float max_advance, float max_turn_angle,float *observations, int size, float laser_value)
{
	/*
 ...
 else
 {
	...
	for(int i=0;i<size;i++){
		...
		if(observations[i]<laser_value){
			...
			if(par_flag==0){
				//Si hay un numero impar de sensores

				if(i<a){
					//Si el sensor está del lado derecho
					thobs=2*mpi-mpi/2+i*mpi/size;
				}else{
					thobs=mpi+(i-6)*mpi/size;
				}

			}else{
				thobs=-mpi/2+i*mpi/size;
				if(thobs<0){
					thobs=2*mpi+thobs;
				}
			}
	...
	Ftot[0]=1.2*Fatr[0]+Frep[0];
	Ftot[1]=1.2*Fatr[1]+Frep[1];
